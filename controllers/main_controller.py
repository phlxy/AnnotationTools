...
class MainController(QObject):
    ...
    def __init__(self):
        ...
        print(f"[DEBUG] MainController: Initialized with LabelModel.")

    def change_document_type(self, document_type: str):
        """เปลี่ยนประเภทเอกสารใน LabelModel"""
        print(f"[DEBUG] MainController: Changing document type to '{document_type}'")
    ...
    def get_all_labels(self) -> List[str]:
        ...
        print(f"[DEBUG] MainController: Retrieved labels {labels}")
        return labels

    def set_current_label(self, label: str):
        """ตั้งค่าป้ายกำกับปัจจุบันใน LabelModel"""
        print(f"[DEBUG] MainController: Setting current label to '{label}'")
